...
//////////////////////////////////////////////////////////////////
enum GameState {WIN, NOTWIN, DRAW}; // Состояние игры
...
bool answer(const char *task){
	std::string answer;
    std::cout << task;
    std::cin >> answer;
	// y, yes, yep, yay, yeah, Y, YES, YEP
	transform(answer.begin(), answer.end(), answer.begin(), ::tolower);
	if (answer.find('y') == 0) return 1;
	else return 0;
}

bool TStart(){
	if (answer("You want to play? ")){
		int winCount = 0;
		Field f;
		while (true) {
			initField(f);
...
void printField(const Field& field) {
    ...
    std::cout << "-x 1 x 2 x 3 x" << std::endl;
    for (int y = 0; y < field.szY; y++) {
        std::cout << (y + 1) << "y | ";
        for (int x = 0; x < field.szX; x++) {
...
void playerMove(Field& field) {
    ...
    do {
        ...
        std::cout << std::endl << " Enter your move coordinates \n X and Y (1 to " << field.szX << ",1 to " << field.szY << ") space separated \n>> ";
        std::cin >> x;
        std::cin >> y;
        //<regex>
